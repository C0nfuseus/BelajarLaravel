...
<<<<<<< HEAD
                    ...
                    <hr>
=======
                    <h4>Register TNDE DisBudPar Jawa Timur</h4><hr>
>>>>>>> parent of b2cc184 (nambah disposisi)
                    <form action="{{ route('auth.save') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="nama_user" placeholder="masukan nama lengkap" value="{{ old('nama_user') }}" >
                        </div>
                        <span class="text-danger">@error ('nama_user'){{ $message }} @enderror</span>
                        <br>
                        <div class="form-group">
                            <label>NIP</label>
                            <input type="number" class="form-control" name="username" placeholder="masukan NIP" value="{{ old('username') }}" >
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="id_bidang" name="id_bidang" class="form-control">
                                ...
                                @foreach ($data1 as $bidang)
                                    <option value="{{ $bidang->id_bidang }}">{{$bidang->id_bidang}} - {{ $bidang->nama_bidang }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="id_role" name="id_role" class="form-control">
                                ...
                                @foreach ($data2 as $role)
                                    <option value="{{ $role->id_role }}">{{ $role->id_role}} - {{ $role->role }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                        <label>jabatan</label>
                        <input type="text" class="form-control" name="jabatan" placeholder="masukan jabatan" value="{{ old('jabatan') }}" >
                        </div>
